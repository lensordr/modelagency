...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run(app, host="0.0.0.0", port=8000)nue),
...
@app.get("/business/analytics/hourly-pattern")
async def get_hourly_pattern(
    ...
    try:
        ...
        return {"hourly_data": [], "error": str(e)}

@app.get("/business/analytics/waiters")
async def get_waiter_stats(
    db: Session = Depends(get_db)
):
    try:
        # Return simple waiter list for now
        waiters = db.query(Waiter).filter(Waiter.active == True).all()
        return {
            "waiters": [
                {
                    "id": waiter.id,
...
async def get_analytics_dashboard(
    ...
    try:
        from simple_analytics import get_simple_top_items, get_simple_categories, get_simple_trends
        
        # Get basic sales data
        sales_data = get_detailed_sales_data(db, 'day', date.today().isoformat())
        top_items = get_simple_top_items(db, 5)
        categories = get_simple_categories(db)
        trends = get_simple_trends(db, 7)
        
        total_cat_revenue = sum(float(c.total_revenue) for c in categories) if categories else 1
            ...
            "categories": [
                {
                    "category": cat.category,
                    "revenue": float(cat.total_revenue),
                    "percentage": float(cat.total_revenue) / total_cat_revenue * 100
                }
                for cat in categories
            ],
            "trends": [
